...
This version adds a Monte-Carlo wrapper for N trials and uses an independent
np.random.Generator for each trial/case so that changing the seed actually
changes the simulated odometry/GPS noise.

Run:
    python main_script_monte_carlo.py
"""
...
def run_case(
    ...
    condition_times_sum = np.zeros(3)
    count = 0
    ...
    for k in range(ct.T - 1):
        ...
        if update_mask[k + 1] and ekf.update_t is not None:
            condition_times_sum += np.array([ekf.update_t, liekf.update_t, iscvx.update_t])
            count += 1

    condition_times_avg = condition_times_sum / count if count > 0 else np.full(3, np.nan)
...
def print_trial_summary(trial_idx: int, results):
    ...
    for res in results:
        ...
        print(
            "  avg update time [EKF, LIEKF, ISCVX]: "
