...
This version adds optional obstacle-avoidance constraints to the CVXPY
conditioning step.  The prediction is exactly the same as the LIEKF prediction,
...
``continuous_discrete_iscvx.py`` only when obstacle constraints are not used.
Obstacle constraints require CVXPY.
"""
...
# It is used only when obstacle constraints are absent.
...
class ContinuousDiscreteCarISCVXCVXPY:
    ...
    Optional obstacle constraints can be passed through ``obs`` and ``obs_r``.
    The obstacle set is interpreted as circular keep-out regions in the x-y
    plane with centers ``obs[j]`` and radius ``obs_r``.
    """
    ...
    def __init__(
        ...
        self.obs_r = None if obs_r is None else float(obs_r)
        ...
        if self.obs is not None and self.obs_r is None:
            raise ValueError("obs_r must be provided when obs is not None.")
...
def _format_obstacles(obs: np.ndarray | None) -> np.ndarray | None:
    """Return obstacles as an array with shape (num_obs, 2), or None."""
    if obs is None:
        return None
    obs_arr = np.asarray(obs, dtype=float)
    if obs_arr.size == 0:
        return None
    obs_arr = obs_arr.reshape(-1, 2)
    return obs_arr
...
def solve_trust_region_qp_cvxpy(
    ...
    When ``obs`` and ``obs_r`` are provided, the QP also contains the linearized
    obstacle constraints

        h_j + a_j^T H_p eta <= 0.

    The unconstrained first stage is used only when no obstacle constraints are
    active.  With obstacle constraints, every candidate step must satisfy those
    constraints, so the problem is solved directly with the constrained form.
    """
    ...
    Ninv = _make_psd(Ninv)
    obs_arr = _format_obstacles(obs)
    has_obstacles = obs_arr is not None

    if has_obstacles and z_lin is None:
        raise ValueError("z_lin must be provided when obstacle constraints are used.")
    ...
    # Stage 1: unconstrained QP. Only safe when no obstacle constraints are active.
    ...
    # Stage 2: trust-region constrained problem, plus optional obstacle constraints.
    eta = cp.Variable(dim)
...
def intrinsic_cvxpy_update_SE2(
    ...
    z_upd, P_upd, used_cvxpy
        ``used_cvxpy`` is False only when cvxpy is absent, the optional direct
        trust-region fallback was used, and obstacle constraints are absent.
    """
    ...
    if obs_arr is not None and obs_r is None:
        raise ValueError("obs_r must be provided when obs is not None.")
    ...
    if not CVXPY_AVAILABLE:
        if obs_arr is not None:
            raise ImportError(
                "Obstacle-constrained conditioning requires cvxpy. "
                "The direct fallback solver does not support obstacle constraints."
            )
